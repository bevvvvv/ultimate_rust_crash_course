...
fn main() {
    ...
    // the child threads.
    let (transmit, receiver) = channel::unbounded();
    let receiver2 = receiver.clone();

    transmit.send("First Message").unwrap();
    transmit.send("Second Message").unwrap();

    let handle_receiver_a = thread::spawn(move || {
        for value in receiver {
            println!("Child thread A: Received {}", value);
            pause_ms(100);
        }
    });
    ...
    transmit.send("Third Message").unwrap();

    drop(transmit);

    handle_receiver_a.join().unwrap();
    handle_receiver_b.join().unwrap();
